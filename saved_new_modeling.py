...
class InternVLChatModel(PreTrainedModel):
    ...
    def visualize_attention_maps(
        ...
        if vision_attentions is not None:
            print("visualizing vision attention")
            self._visualize_vision_attention(vision_attentions, pixel_values, save_path)
        
        # Visualize Language Model attention
        #if language_attentions is not None:
            #print("visualizing language attention")
            #self._visualize_language_attention(language_attentions, input_ids, save_path)
        
        print(f"Attention maps saved to {save_path}")

    def _visualize_vision_attention(self, vision_attentions, pixel_values, save_path):
        """
        ...
        for layer_idx, attention in enumerate(vision_attentions):
            print("layer_idx:", layer_idx)
            # attention shape: [batch_size, num_heads, seq_len, seq_len]
            attention = attention.detach().cpu().float().numpy()
            
            for batch_idx in range(min(batch_size, 2)):  # Visualize first 2 images
                # Average across attention heads
                print("batch_idx:", batch_idx)
                avg_attention = attention[batch_idx].mean(axis=0)
                
                # Remove CLS token (first token) from visualization
                patch_attention = avg_attention[1:, 1:]  # Remove first row and column
                
                # Reshape to spatial dimensions
                grid_size = int(np.sqrt(patch_attention.shape[0]))
                print("grid_size:", grid_size)
                print("patch_attention shape:", patch_attention.shape[0])
                if grid_size * grid_size == patch_attention.shape[0]:
                    print("plotting image")
                    spatial_attention = patch_attention.mean(axis=1).reshape(grid_size, grid_size)
                    print("spatial_attention shape:", spatial_attention.shape)
                    plt.figure(figsize=(8, 6))
                    if sns is not None:
                        sns.heatmap(spatial_attention, annot=False, cmap='viridis')
                    else:
                        plt.imshow(spatial_attention, cmap='viridis')
                        plt.colorbar()
                    plt.title(f'Vision Layer {layer_idx} - Batch {batch_idx} - Spatial Attention')
                    plt.savefig(f'{save_path}/vision_layer_{layer_idx}_batch_{batch_idx}_spatial.png', 
                               dpi=300, bbox_inches='tight')
                    plt.close()
                
                # Visualize attention to CLS token (global attention)
                cls_attention = avg_attention[0, 1:].reshape(grid_size, grid_size)
                plt.figure(figsize=(8, 6))
                if sns is not None:
                    sns.heatmap(cls_attention, annot=False, cmap='plasma')
                else:
                    plt.imshow(cls_attention, cmap='plasma')
                    plt.colorbar()
                plt.title(f'Vision Layer {layer_idx} - Batch {batch_idx} - CLS Token Attention')
                plt.savefig(f'{save_path}/vision_layer_{layer_idx}_batch_{batch_idx}_cls.png', 
                           dpi=300, bbox_inches='tight')
                plt.close()

    def _visualize_language_attention(self, language_attentions, input_ids, save_path):
        """
        Visualize Language Model attention maps
        """
        ...
        for layer_idx, attention in enumerate(language_attentions):
            ...
            for batch_idx in range(min(batch_size, 2)):  # Visualize first 2 sequences
                # Average across attention heads
                avg_attention = attention[batch_idx].mean(axis=0)
                ...
                if sns is not None:
                    sns.heatmap(avg_attention, annot=False, cmap='Blues', square=True)
                else:
                    plt.imshow(avg_attention, cmap='Blues')
                    plt.colorbar()
                plt.title(f'Language Layer {layer_idx} - Batch {batch_idx} - Token Attention')
                plt.xlabel('Key Position')
                plt.ylabel('Query Position')
                plt.savefig(f'{save_path}/language_layer_{layer_idx}_batch_{batch_idx}.png', 
                           dpi=300, bbox_inches='tight')
                plt.close()
                
                # Visualize attention patterns for image tokens specifically
                if hasattr(self, 'img_context_token_id') and self.img_context_token_id is not None:
                    ...
                    if len(img_token_positions) > 0:
                        # Average attention from all positions to image tokens
                        img_attention = avg_attention[:, img_token_positions].mean(axis=1)
                        
                        plt.figure(figsize=(15, 4))
                        plt.bar(range(len(img_attention)), img_attention)
                        plt.title(f'Language Layer {layer_idx} - Batch {batch_idx} - Attention to Image Tokens')
                        plt.xlabel('Token Position')
                        plt.ylabel('Average Attention Weight')
                        
                        # Mark image token positions
                        for pos in img_token_positions:
                            plt.axvline(x=pos, color='red', linestyle='--', alpha=0.7)
                        
                        plt.savefig(f'{save_path}/language_layer_{layer_idx}_batch_{batch_idx}_img_attention.png', 
                                   dpi=300, bbox_inches='tight')
                        plt.close()
